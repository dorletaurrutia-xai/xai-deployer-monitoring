...
- Markdown report saved to analysis_history/facial_recognition_analysis_YYYYMMDD.md

Failure mode: demographic_proxy_alert — demographic-correlated proxy features
(historical false-positive ratio, skin-tone estimate, prior watch-list flag)
contaminate the alert-activation decision. The instrument recovers how much
of each alert decision traces to demographic proxies vs. legitimate match-
quality signals — the per-case evidence absent from real deployments.

Primary signal:   constraint_enforcement (demographic proxy attribution)
Secondary signal: decision_robustness    (flip-rate near vs. far)

Industry 5.0 thresholds:
  sustainability FLAG  if demographic_parity_ratio attribution > 0.15
  human_ai_teaming:
...
from cat1_facial_recognition.generate_dataset import (
    generate_fr_dataset,
    FEATURE_NAMES,
    G_DOMAIN,
    G_PROXY,
    DEMOGRAPHIC_PROXY_IDX,
    CODED_EFFECTS,
)
from instrumentation.protocol import InstrumentationProtocol

ANALYSIS_HISTORY_DIR = REPO_DIR.parent / "xai-analysis-history" / "cat1_facial_recognition"
...
def sustainability_signal(proxy_attr: dict) -> dict:
    """FLAG if demographic_parity_ratio attribution > 0.15."""
    dpr_val = proxy_attr.get("demographic_parity_ratio", 0.0)
    return {
        "dimension": "civil_liberties",
        "at_risk_group": "non_white_subjects",
        "indicator": "demographic_proxy_alert_rate",
        "value": round(dpr_val, 4),
        "status": "FLAG" if dpr_val > 0.15 else "OK",
    }
...
def run_analysis(n_cases: int = 40) -> dict:
    ...
    for label, indices, is_near in [
        ...
        for j in indices:
            ...
            snippet = protocol.generate_snippet(
                x,
                case_id=f"facial_recognition-{label}-{int(j):04d}",
                tau_stab=0.3,
            )
    ...
    for lbl in ["near", "far"]:
        ...
        if cases:
            ...
            # demographic_parity_ratio attribution specifically
            results[f"{lbl}_dpr_mean"]    = float(np.mean([
                c["constraint_enforcement"]["proxy_detail"].get("demographic_parity_ratio", 0)
                for c in cases
            ]))
            results[f"{lbl}_dpr_std"]     = float(np.std([
                c["constraint_enforcement"]["proxy_detail"].get("demographic_parity_ratio", 0)
                for c in cases
            ]))
...
def build_markdown(res: dict, today: str) -> str:
    near = res["near_cases"]
    far  = res["far_cases"]
    # Prefer a case where sustainability_impact is FLAG for the evidence snippet
    ...
    lines = [
        "# Cat. 1 — Biometric Identification: Facial Recognition Demographic Proxy Alert",
        f"<!-- Generated: {today} | failure_mode: demographic_proxy_alert -->",
        "",
        "---",
        "",
        "**Failure mode:** `demographic_proxy_alert` — demographic-correlated proxy "
        "features (historical false-positive ratio, skin-tone estimate, prior "
        "watch-list flag) contaminate the alert-activation decision in live facial "
        "recognition. Inspired by Metropolitan Police UK LFR deployments and the "
        "Big Brother Watch (2018) audit documenting 98% false positive rates.",
        "",
        "**Primary signal:** `constraint_enforcement` (KernelSHAP attribution).",
        "**Regulation:** EU AI Act Annex III Category 1 (remote biometric identification).",
        "",
        "### Calibration results",
        "",
        ...
        f"| Mean DPR attribution | {res.get('near_dpr_mean',0):.3f} ± {res.get('near_dpr_std',0):.3f} | {res.get('far_dpr_mean',0):.3f} ± {res.get('far_dpr_std',0):.3f} |",
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Cat. 1 Instrumentation Analysis — Facial Recognition")
    print("=" * 60)
    ...
    print(f"  DPR   near={res.get('near_dpr_mean',0):.3f}±{res.get('near_dpr_std',0):.3f}  "
          f"far={res.get('far_dpr_mean',0):.3f}±{res.get('far_dpr_std',0):.3f}")
    ...
    out_path = ANALYSIS_HISTORY_DIR / f"facial_recognition_analysis_{today}.md"
    out_path.write_text(build_markdown(res, today), encoding="utf-8")
    print(f"\nReport saved → {out_path}")

    print("\n── Example snippet (near-threshold, sustainability FLAG preferred) ──")
