"""Cat. 5 — Access to Essential Services: HELOC Credit Score-on-Score Opacity.

Produces:
- Console calibration summary
- JSON evidence snippets with 7 signals (5 protocol + 2 Industry 5.0)
- Markdown report saved to analysis_history/heloc_credit_analysis_YYYYMMDD.md

Failure mode: score_on_score_opacity — an opaque external bureau score
(external_risk_estimate) dominates the credit decision. The deployer cannot
decompose what drives the bureau score, yet it carries ~20% of the approval
attribution — a black-box inside a black-box.

Primary signal:   constraint_enforcement (bureau score attribution)
Secondary signal: decision_robustness    (flip-rate near vs. far)

Industry 5.0 thresholds:
  sustainability FLAG  if proxy_attribution_total > 0.20
  human_ai_teaming:
...
ANALYSIS_HISTORY_DIR = REPO_DIR.parent / "xai-analysis-history" / "cat5_heloc_credit"
...
def sustainability_signal(proxy_total: float) -> dict:
    """FLAG if bureau score attribution > 0.20 (coded: ~20.3% overall)."""
    return {
        "dimension": "financial_inclusion",
        "at_risk_group": "credit_applicants",
        "indicator": "bureau_score_opacity_rate",
        "value": round(proxy_total, 4),
        "status": "FLAG" if proxy_total > 0.20 else "OK",
    }
...
def human_ai_teaming_signal(fr_val: float, is_near: bool) -> dict:
    ...
    return {
        "dimension": "underwriter_oversight",
        "escalation_level": level,
        "trigger": "near_threshold_bureau_score_dominance",
        "human_override_available": True,
        "note": "HELOC: near-threshold credit decisions with opaque bureau score require underwriter review",
        "status": status,
    }
...
def run_analysis(n_cases: int = 40) -> dict:
    ...
    results = {
        "demonstrator": "heloc_credit",
        "failure_mode": "score_on_score_opacity",
        "annex_iii_category": "cat_5",
        "accuracy": acc,
    ...
    for label, indices, is_near in [
        ...
        for j in indices:
            ...
            snippet = protocol.generate_snippet(
                x,
                case_id=f"heloc_credit-{label}-{int(j):04d}",
                tau_stab=0.3,
            )
    ...
    for lbl in ["near", "far"]:
        ...
        if cases:
            ...
            # bureau score attribution specifically (sole proxy)
            results[f"{lbl}_bureau_mean"] = float(np.mean([
                c["constraint_enforcement"]["proxy_detail"].get("external_risk_estimate", 0)
                for c in cases
            ]))
            results[f"{lbl}_bureau_std"]  = float(np.std([
                c["constraint_enforcement"]["proxy_detail"].get("external_risk_estimate", 0)
                for c in cases
            ]))
...
def build_markdown(res: dict, today: str) -> str:
    ...
    lines = [
        "# Cat. 5 — Access to Services: HELOC Credit Score-on-Score Opacity",
        f"<!-- Generated: {today} | failure_mode: score_on_score_opacity -->",
        "",
        "---",
        "",
        "**Failure mode:** `score_on_score_opacity` — an opaque external bureau score "
        "dominates the credit decision. The deployer cannot decompose what drives the "
        "bureau score, yet it carries ~20% of the approval attribution near the decision "
        "boundary — a black-box inside a black-box. Inspired by FICO HELOC data.",
        "",
        "**Primary signal:** `constraint_enforcement` (KernelSHAP attribution).",
        "**Regulation:** EU AI Act Annex III Category 5 (access to essential services).",
        "",
        "### Calibration results",
        "",
        ...
        f"| Mean bureau score attribution | {res.get('near_bureau_mean',0):.3f} ± {res.get('near_bureau_std',0):.3f} | {res.get('far_bureau_mean',0):.3f} ± {res.get('far_bureau_std',0):.3f} |",
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Cat. 5 Instrumentation Analysis — HELOC Credit")
    print("=" * 60)
    ...
    print(f"  Bureau near={res.get('near_bureau_mean',0):.3f}±{res.get('near_bureau_std',0):.3f}  "
          f"far={res.get('far_bureau_mean',0):.3f}±{res.get('far_bureau_std',0):.3f}")
    ...
    out_path = ANALYSIS_HISTORY_DIR / f"heloc_credit_analysis_{today}.md"
