"""Cat. 8 — COMPAS Sentencing: Opacity in Sentencing Instrumentation Analysis.

Produces:
- Console calibration summary
- JSON evidence snippets with 7 signals (5 protocol + 2 Industry 5.0)
- Markdown report saved to analysis_history/compas_sentencing_analysis_YYYYMMDD.md

Failure mode: opacity_in_sentencing — race and structural proxy features
contaminate the sentencing risk recommendation. The defendant has no access
to the attribution evidence this instrument produces (Loomis v. Wisconsin).

Primary signal:   constraint_enforcement (race + proxy attribution)
Secondary signal: decision_robustness    (flip-rate near vs. far)

Industry 5.0 thresholds:
  sustainability FLAG  if race_attribution (race_encoded) > 0.10
  human_ai_teaming:
...
from cat8_compas_sentencing.generate_dataset import (
    generate_sentencing_dataset,
    FEATURE_NAMES,
    G_DOMAIN,
    G_PROXY,
    RACE_FEATURE_IDX,
...
ANALYSIS_HISTORY_DIR = REPO_DIR.parent / "xai-analysis-history" / "cat8_compas_sentencing"
...
def sustainability_signal(proxy_attr: dict) -> dict:
    """FLAG if race_encoded attribution > 0.10."""
    race_val = proxy_attr.get("race_encoded", 0.0)
    return {
    ...
    for label, indices, is_near in [
        ...
        for j in indices:
            ...
            snippet = protocol.generate_snippet(
                x,
                case_id=f"compas_sentencing-{label}-{int(j):04d}",
                tau_stab=0.3,
            )
    ...
    for lbl in ["near", "far"]:
        ...
        if cases:
            ...
            results[f"{lbl}_race_mean"]   = float(np.mean([
                c["constraint_enforcement"]["proxy_detail"].get("race_encoded", 0)
                for c in cases
            ]))
...
def build_markdown(res: dict, today: str) -> str:
    ...
    lines = [
        ...
        f"<!-- Generated: {today} | failure_mode: opacity_in_sentencing -->",
        "",
        "---",
        "",
        "**Failure mode:** `opacity_in_sentencing` — race and structural proxy features "
        "contaminate the algorithmic sentencing recommendation. Inspired by Loomis v. "
        "Wisconsin (2016): judicial use permitted despite algorithmic opacity.",
        "",
        "**Primary signal:** `constraint_enforcement` (KernelSHAP attribution).",
        "**Regulation:** EU AI Act Annex III Category 8 (administration of justice).",
        "",
        "### Calibration results",
        "",
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Cat. 8 Instrumentation Analysis — COMPAS Sentencing")
    print("=" * 60)
    ...
    out_path = ANALYSIS_HISTORY_DIR / f"compas_sentencing_analysis_{today}.md"
