"""Brake Disc Quality Control — Synthetic Dataset Generator (subcaso Cat. 2).
...
This file is the Cat. 2 sub-case integrated into cobot_safety/.
The principal case (cobot) tests constraint_enforcement (identity contamination).
This sub-case tests decision_robustness (confounder-driven fragility).

Dataset design criteria
-----------------------
1. FEATURE FIDELITY: Features named and typed as in a real brake disc
...
Scale note
----------
Logistic scale set to 7 (vs. 4 in the standalone brake_disc/ demonstrator)
to ensure sufficient far-threshold cases (≥35) for Mann-Whitney U discrimination
tests. The coded attribution ratios are unaffected by the scale parameter.
"""
...
def generate_brake_disc_dataset(
    ...
    Note: 'shift' is a construction variable (metadata only) — it is NOT
    in FEATURE_NAMES and does not enter the model directly.
    """
    ...
    # shift is a construction variable — correlated with confounders but
    # excluded from FEATURE_NAMES (not visible to the model).
    ...
    # Scale=7: produces sufficient far-threshold cases for discrimination tests.
    combined = defect_signal + confounder_signal
    noise = rng.normal(0, 0.08, n)
    prob_reject = 1 / (1 + np.exp(-(combined + noise - 0.5) * 7))

    label = (rng.random(n) < (1 - prob_reject)).astype(int)  # 1 = pass

    # Margin to threshold (metadata — used for near/far split diagnostics)
    ...
    df = pd.DataFrame({
        ...
        # Label and metadata (shift excluded from FEATURE_NAMES)
        "label": label,
        "shift": shift,
...
# NOTE: 'shift' is intentionally excluded — it is metadata, not a model input.
FEATURE_NAMES = [
...
# k-NN envelope reference: day-shift cases only (shift == 0).
# Day-shift represents the operational baseline: normal lighting, minimal oil
# residue, stable camera angle. Night-shift cases carry elevated confounders
# and must appear out-of-distribution relative to this reference — that is
# exactly the signal the envelope_validity sensor is calibrated to detect.
KNN_REFERENCE_COL = "shift"
KNN_REFERENCE_VAL = 0  # 0 = day shift

# k-NN envelope subspace: compute distance only in the confounder dimensions.
# Legitimate features (crack depth, runout, etc.) are orthogonal to shift and
# create noise in the envelope calculation — extreme-defect far-threshold cases
# happen to have unusual legitimate features but normal confounders.
# Restricting to G_PROXY ensures the envelope measures whether the *environmental
# conditions* are within the operational baseline, which is the correct question
# for detecting confounder-driven out-of-distribution inputs.
KNN_SUBSPACE_INDICES = G_PROXY  # [9, 10, 11, 12]
...
if __name__ == "__main__":
    ...
    df.to_csv(out_dir / "brake_disc_qc.csv", index=False)
    print(f"Generated {len(df)} samples -> {out_dir / 'brake_disc_qc.csv'}")
