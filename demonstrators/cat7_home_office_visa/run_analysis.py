"""Cat. 7 — Home Office Visa Streaming: Nationality Proxy Instrumentation.

Produces:
- Console calibration summary
- JSON evidence snippets with 7 signals (5 protocol + 2 Industry 5.0)
- Markdown report saved to analysis_history/home_office_visa_analysis_YYYYMMDD.md

Failure mode: nationality_proxy_streaming — an opaque nationality risk score
(legacy streaming system output) dominates the visa approval decision.
Black-box-inside-a-black-box: the deployer cannot decompose the score, yet
it carries ~30% of the approval attribution near the decision boundary.

Primary signal:   constraint_enforcement (nationality_risk_score attribution)
Secondary signal: decision_robustness    (flip-rate near vs. far)

Industry 5.0 thresholds:
  sustainability FLAG  if nationality_risk_score attribution > 0.20
  human_ai_teaming:
...
from cat7_home_office_visa.generate_dataset import (
    generate_visa_dataset,
    FEATURE_NAMES,
    G_DOMAIN,
    G_PROXY,
    NRS_FEATURE_IDX,
...
ANALYSIS_HISTORY_DIR = REPO_DIR.parent / "xai-analysis-history" / "cat7_home_office_visa"
...
def sustainability_signal(proxy_attr: dict) -> dict:
    """FLAG if nationality_risk_score attribution > 0.20."""
    nrs_val = proxy_attr.get("nationality_risk_score", 0.0)
    return {
        "dimension": "migration_equity",
        "at_risk_group": "high_tier_nationalities",
        "indicator": "nationality_proxy_rejection_rate",
        "value": round(nrs_val, 4),
        "status": "FLAG" if nrs_val > 0.20 else "OK",
    }
...
def run_analysis(n_cases: int = 40) -> dict:
    ...
    for label, indices, is_near in [
        ...
        for j in indices:
            ...
            snippet = protocol.generate_snippet(
                x,
                case_id=f"home_office_visa-{label}-{int(j):04d}",
                tau_stab=0.3,
            )
    ...
    for lbl in ["near", "far"]:
        ...
        if cases:
            ...
            # nationality_risk_score attribution specifically
            results[f"{lbl}_nrs_mean"]    = float(np.mean([
                c["constraint_enforcement"]["proxy_detail"].get("nationality_risk_score", 0)
                for c in cases
            ]))
            results[f"{lbl}_nrs_std"]     = float(np.std([
                c["constraint_enforcement"]["proxy_detail"].get("nationality_risk_score", 0)
                for c in cases
            ]))
...
def build_markdown(res: dict, today: str) -> str:
    ...
    lines = [
        ...
        f"<!-- Generated: {today} | failure_mode: nationality_proxy_streaming -->",
        "",
        "---",
        "",
        "**Failure mode:** `nationality_proxy_streaming` — an opaque nationality "
        "risk score (legacy streaming system output) dominates the visa approval "
        "decision. Inspired by the Home Office UK streaming tool (2015–2020), "
        "withdrawn after Foxglove/JCWI 2020 audit.",
        "",
        "**Primary signal:** `constraint_enforcement` (KernelSHAP attribution).",
        "**Regulation:** EU AI Act Annex III Category 7 (migration and border management).",
        "",
        "### Calibration results",
        "",
        ...
        f"| Mean NRS attribution | {res.get('near_nrs_mean',0):.3f} ± {res.get('near_nrs_std',0):.3f} | {res.get('far_nrs_mean',0):.3f} ± {res.get('far_nrs_std',0):.3f} |",
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Cat. 7 Instrumentation Analysis — Home Office Visa")
    print("=" * 60)
    ...
    print(f"  NRS   near={res.get('near_nrs_mean',0):.3f}±{res.get('near_nrs_std',0):.3f}  "
          f"far={res.get('far_nrs_mean',0):.3f}±{res.get('far_nrs_std',0):.3f}")
    ...
    out_path = ANALYSIS_HISTORY_DIR / f"home_office_visa_analysis_{today}.md"
